...
import { build2hReminderMessage } from '@/lib/notifications/build-reminder-message'
import type { Lang } from '@/lib/whatsapp/templates'

/**
 * Sends WhatsApp 2-hour appointment reminder (FR-23, Story 7.1).
 * Text-only message with "Reply CANCEL to cancel" instruction.
 * Checks: appointment confirmed + clinic toggle enabled + patient not opted out.
 */
export const appointmentReminder2h = inngest.createFunction(
  { id: 'appointment-reminder-2h', name: 'Appointment: Send 2h Reminder' },
  { event: 'appointment/reminder-2h.send' },
  async ({ event }) => {
    ...
    if (!clinic.reminder2hEnabled) return { sent: false, reason: 'toggle_disabled' }
    ...
    const aptRows = await db.$queryRawUnsafe<{
      ...
      `SELECT status, token_number, appointment_date::text, appointment_time::text,
              is_sample, patient_id, doctor_id
      ...
      await db.$executeRawUnsafe(
        `UPDATE "${schemaName}".appointments
         SET delivery_failures = COALESCE(delivery_failures, '[]'::jsonb) || $1::jsonb,
             updated_at = NOW()
         WHERE id = $2::uuid`,
        JSON.stringify([{ channel: 'whatsapp-skipped-optout', reminderType: '2h', at: new Date().toISOString() }]),
        appointmentId
      )
      await inngest.send({
        name: 'appointment/sms-fallback.send',
        data: { appointmentId, clinicId, channel: 'reminder-2h' },
      })
    ...
    const message = build2hReminderMessage({
    ...
    if (result.success) {
      await db.$executeRawUnsafe(
        `UPDATE "${schemaName}".appointments
         SET reminder_2h_sent_at = NOW(), updated_at = NOW()
         WHERE id = $1::uuid`,
        appointmentId
      )
    } else {
      await db.$executeRawUnsafe(
        `UPDATE "${schemaName}".appointments
         SET delivery_failures = COALESCE(delivery_failures, '[]'::jsonb) || $1::jsonb,
             updated_at = NOW()
         WHERE id = $2::uuid`,
        JSON.stringify([{ channel: 'whatsapp-reminder-2h', failedAt: new Date().toISOString(), reason: result.error }]),
        appointmentId
      )
      await inngest.send({
        name: 'appointment/sms-fallback.send',
